...
def evaluate_latent_expressiveness(model, train_loader, val_loader, test_loader, device, save_dir):
    ...
    if has_zay:
        # 3. Hospital classification using za+zay
        ...
        # 4. Hospital classification using zay alone (for comparison)
    ...
    # 2. Tumor classification using za alone (cross-prediction test)
    print("   Training za → tumor classifier...")
    ...
    results['tumor_za_alone'] = {'train_acc': za_t_train_acc, 'val_acc': za_t_val_acc, 'test_acc': za_t_test_acc}

    if has_zay:
        # 3. Tumor classification using zy+zay
        ...
        # 4. Tumor classification using zay alone (for comparison)
    ...
    print(f"   za alone:      Train={results['tumor_za_alone']['train_acc']:.4f}, Val={results['tumor_za_alone']['val_acc']:.4f}, Test={results['tumor_za_alone']['test_acc']:.4f} [cross-prediction]")
    if has_zay:
...
def create_expressiveness_visualization(results, save_dir, has_zay):
    ...
    for split, split_name in [('val_acc', 'Validation'), ('test_acc', 'Test')]:
        ...
        comparison_data.append({
            'Task': 'Tumor Classification',
            'Method': 'za alone',
            'Accuracy': results['tumor_za_alone'][split],
            'Type': 'Cross-prediction',
            'Split': split_name
        })
